...
// ════════════════════════════════════════════════
// PERIODE bepalen (voor sectie 1 en 4)
// ════════════════════════════════════════════════
switch ($periode) {
    case 'week':
...
// ════════════════════════════════════════════════
// SECTIE 1: Wat je al bereikt hebt
// ════════════════════════════════════════════════
$stmt = $pdo->prepare(
    "SELECT
        COUNT(*) AS aantal,
        COALESCE(SUM(duur_minuten), 0) AS totaal_minuten
     FROM trainings
...
// Gewicht historiek
$stmt = $pdo->prepare(
    "SELECT gewicht_kg, gemeten_op
     FROM body_weight WHERE user_id = ?
     ORDER BY gemeten_op ASC"
);
$stmt->execute([$user_id]);
$gewichtData = $stmt->fetchAll();

// ════════════════════════════════════════════════
// SECTIE 3: Streaks + per-week grafiek
// ════════════════════════════════════════════════
$stmt = $pdo->prepare(
...
// Trainingen per week sinds eerste training
$stmt = $pdo->prepare(
    "SELECT
        YEARWEEK(datum, 1) AS jaarweek,
        MIN(datum) AS week_start,
        COUNT(*) AS aantal
     FROM trainings
     WHERE user_id = ?
     GROUP BY YEARWEEK(datum, 1)
     ORDER BY jaarweek ASC"
);
$stmt->execute([$user_id]);
$rawPerWeek = $stmt->fetchAll();

// Vul gaten op: weken zonder training tussen eerste en laatste week
$trainingenPerWeek = [];
if (!empty($rawPerWeek)) {
    $lookup = [];
    foreach ($rawPerWeek as $row) {
        $lookup[$row['jaarweek']] = (int) $row['aantal'];
    }
    ...
    while ($cursor <= $vandaagMaandag) {
        $jw = $cursor->format('oW'); // ISO year + week
...
if (count($gewichtData) >= 2) {
    $eerste = $gewichtData[0]['gewicht_kg'];
    $laatste = end($gewichtData)['gewicht_kg'];
    $gewichtsVerandering = round($laatste - $eerste, 1);
}
...
$gemDuur = $sectie1['aantal'] > 0
    ? round($sectie1['totaal_minuten'] / $sectie1['aantal'])
    : 0;
    ...
    <details class="stat-sectie" open>
        ...
        <div class="sectie-inhoud">
            <div class="stat-grid-2">
                <div class="stat-kaart">
                    ...
                    <?php if ($goals['weekly_sessions']): ?>
                        ...
                        <div class="progress-bar">
                            <div class="progress-fill" style="width: <?= percent($weekStats['aantal'], $goals['weekly_sessions']) ?>%;"></div>
                        </div>
                        <p class="hint">
                            <?php
                                $nog = (int) $goals['weekly_sessions'] - (int) $weekStats['aantal'];
                                if ($nog <= 0) echo "Doel behaald! 🎉";
                                else echo "Nog $nog te gaan deze week";
                            ?>
                        </p>
                    <?php else: ?>
                ...
                <div class="stat-kaart">
                    ...
                    <?php if ($goals['weekly_minutes']): ?>
                        ...
                        <div class="progress-bar">
                            <div class="progress-fill" style="width: <?= percent($weekStats['minuten'], $goals['weekly_minutes']) ?>%;"></div>
                        </div>
                        <p class="hint">
                            <?php
                                $nog = (int) $goals['weekly_minutes'] - (int) $weekStats['minuten'];
                                if ($nog <= 0) echo "Doel behaald! 🎉";
                                else echo "Nog $nog minuten te gaan";
                            ?>
                        </p>
                    <?php else: ?>
                        <p class="hint">Stel een doel in op de homepage</p>
            ...
            <?php if (count($gewichtData) > 1): ?>
                <div class="stat-kaart-breed" style="margin-top:20px;">
                    ...
                    <?php endif; ?>
                    <canvas id="gewichtGrafiek"></canvas>
                </div>
            <?php else: ?>
...
(() => {
    ...
    const datasets = [{
        ...
        backgroundColor: 'rgba(0,124,240,0.15)',
        fill: true,
        tension: 0.3,
        pointBackgroundColor: racingRed,
        pointRadius: 4,
    }];
    if (data.doel) {
        datasets.push({
            label: 'Doelgewicht',
            data: new Array(data.data.length).fill(data.doel),
            borderColor: racingRed,
            borderDash: [6, 4],
            pointRadius: 0,
            fill: false,
        });
...
// ─── Trainingen per week (staafdiagram) ───
<?php if (count($trainingenPerWeek) > 0): ?>
(() => {
...
(() => {
    ...
    const kleuren = [
        '#ff003c', '#007cf0', '#ff6b00', '#00d4aa',
        '#9b59b6', '#f1c40f', '#e74c3c', '#3498db',
    ...
    new Chart(document.getElementById('workoutDonut'), {
        ...
        data: {
            ...
            datasets: [{
                data: data.data,
                backgroundColor: kleuren.slice(0, data.labels.length),
                borderColor: '#0d1520',
                borderWidth: 2,
            }]
        },
